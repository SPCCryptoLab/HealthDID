...
void Send(NetIO &io, PP &pp, std::vector<block> &vec_X,std::vector<ECPoint> UID) 
{
    ...
    ALSZOTE::OnesidedSend(io, pp.ote_part, vec_X, vec_X,pp.SENDER_LEN,pp.SENDER_LEN); 
...
void Send(NetIO &io, PP &pp, std::vector<std::vector<uint8_t>> &vec_X, size_t ITEM_LEN,std::vector<ECPoint> UID) 
{
    if(vec_X.size() != pp.SENDER_LEN){
        std::cerr << "|X| does not match public parameter" << std::endl; 
        exit(1); 
    ...
    cwPRFmqRPMT::Client(io, pp.mqrpmt_part, vec_Block_X,UID);
...
std::vector<std::vector<uint8_t>> Receive(NetIO &io, PP &pp, std::vector<std::vector<uint8_t>> &vec_Y, size_t ITEM_LEN) 
{
    ...
    BigInt sk_c=GenRandomBigIntLessThan(order); 
    std::vector<uint8_t> vec_indication_bit = cwPRFmqRPMT::Server(io, pp.mqrpmt_part, vec_Block_Y,sk_c);
