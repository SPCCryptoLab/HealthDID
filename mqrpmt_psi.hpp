...
/*void Send_UID(NetIO &io, PP &pp, std::vector<ECPoint> UID) 
{
   std::cout << "[UID] Phase 0:Send UID>>>" << std::endl;
   cwPRFmqRPMT::Send_UID_Server(io, pp.mqrpmt_part, UID);

}*/

void Send(NetIO &io, PP &pp, std::vector<block> &vec_X,std::vector<block> &vec_D,std::vector<ECPoint> UID) 
{
    ...
    ALSZOTE::OnesidedSend(io, pp.ote_part, vec_X,vec_D,vec_X.size(),vec_D.size()); 
...
void Send_Token(NetIO &io, PP &pp, std::vector<ECPoint> Token) 
{
   std::cout << "[Token] Phase 0:Send Token>>>" << std::endl;
   cwPRFmqRPMT::Send_Token_Client(io, pp.mqrpmt_part, Token);

}
std::vector<block> Receive(NetIO &io, PP &pp, std::vector<block> &vec_Y,BigInt sk_c) 
{
