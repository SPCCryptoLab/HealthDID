...
int main()
{
    ...
    if(party == "server"){
        ...
        vec_indication_bit_prime = cwPRFmqRPMT::Server(server, pp, testcase.vec_Y,sk_c);
    ...
    std::vector<ECPoint> UID (LOG_CLIENT_LEN);

    if(party == "client")
    {
        NetIO client("client", "127.0.0.1", 8080);  
        cwPRFmqRPMT::Client(client, pp, testcase.vec_X,UID);   
    } 
